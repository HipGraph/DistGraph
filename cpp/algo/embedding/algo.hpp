...
namespace distblas::algo {
    ...
    public:
        ...
        void algo_force2_vec_ns(int iterations, int batch_size, int ns, VALUE_TYPE lr) {
            ...
            for (int i = 0; i < iterations; i++) {
                ...
                if (alpha > 0 and grid->col_world_size > 1 and i == 0) {
                    ...
                    int last_proc = this->execute_push_model_computations(0, 0, batches,
                                                                          this->data_comm_cache[0].get(), csr_block,
                                                                          batch_size,
                                                                          last_batch_size, considering_batch_size, lr,
                                                                          prevCoordinates, 0, 0,
                                                                          considering_batch_size, false);

                    if (alpha < 1.0) {
                        int prev_start = get_end_proc(1, alpha, grid->col_world_size);
                        this->execute_pull_model_computations(0, 0,
                                                              this->data_comm_cache[0].get(), csr_block, batch_size,
                                                              considering_batch_size, lr, prevCoordinates, prev_start,
                                                              false,
                                                              last_proc, true);
                    }
                }
                ...
                for (int j = 0; j < batches; j++) {
                    ...
                    vector <INDEX_TYPE> random_number_vec = generate_random_numbers(0, (this->sp_local_receiver)->gRows,
                                                                                    seed, ns);
                    ...
                    if (grid->col_world_size == 1) {
                        ...
                        repulsive_comm.get()->transfer_dense_data(random_number_vec, i, j);
                        this->calc_t_dist_replus_rowptr(prevCoordinates, random_number_vec,
                                                        lr, j, batch_size,
                                                        considering_batch_size);
                        ...
                        if (alpha == 0) {
                            ...
                            if (!(i == iterations - 1 and j == batches - 1)) {
                                ...
                                int last_proc = this->execute_push_model_computations(i, j, batches,
                                                                                      this->data_comm_cache[j].get(),
                                                                                      csr_block, batch_size,
                                                                                      last_batch_size,
                                                                                      considering_batch_size, lr,
                                                                                      prevCoordinates,
                                                                                      next_batch_id, next_iteration,
                                                                                      next_considering_batch_size,
                                                                                      true);

                                if (alpha < 1.0) {
                                    int prev_start = get_end_proc(1, alpha, grid->col_world_size);
                                    this->execute_pull_model_computations(next_iteration,
                                                                          next_batch_id,
                                                                          this->data_comm_cache[next_batch_id].get(),
                                                                          csr_block, batch_size, considering_batch_size,
                                                                          lr,
        ...
        inline int execute_push_model_computations(int iteration,
                                                   int batch, int batches, DataComm<INDEX_TYPE, VALUE_TYPE> *data_comm,
                                                   ...
                                                   int next_batch_id, int next_iteration,
                                                   int next_considering_batch_size,
                                                   bool communication) {
            ...
            for (int k = 1; k < alpha_proc_length; k += alpha_cyc_len) {
                ...
                if (communication) {
                    data_comm->transfer_dense_data(iteration, batch, k, (k + alpha_cyc_len), false);
                }
            ...
            for (int i = 0; i < block_size; i++) {
                ...
                for (int j = 0; j < col_ids.size(); j++) {
                    ...
                    if (fetch_from_cache) {
                        ...
                        VALUE_TYPE *  array_ptr = new VALUE_TYPE [embedding_dim];
                        array_ptr =
                                arrayMap[global_col_id].values.data();
