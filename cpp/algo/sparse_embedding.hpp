...
//                                          considering_batch_size,this->sparse_local_output);
      ...
      cout<<" rank "<<this->grid->rank_in_col<<" before size "<<(*(this->sparse_local->csr_local_data)).handler->rowStart.size()<<"nnz "<<(*(this->sparse_local->csr_local_data)).handler->rowStart[(*(this->sparse_local->csr_local_data)).handler->rowStart.size()-1]<<endl;
          ...
          if (dst_id >= dst_start_index and dst_id < dst_end_index) {
            ...
            if (!fetch_from_cache) {
              ...
              if (symbolic) {
                ...
                CSRHandle *local_handle =
                    this->sparse_local->csr_local_data->handler.get();
                int local_count = local_handle->rowStart[index + 1] -
                                  local_handle->rowStart[index];
