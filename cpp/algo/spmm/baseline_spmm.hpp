...
namespace distblas::algo {
    ...
    public:
        ...
        SpMMAlgo(distblas::core::SpMat<INDEX_TYPE,VALUE_TYPE> *sp_local_native,
                 distblas::core::SpMat<INDEX_TYPE,VALUE_TYPE> *sp_local_receiver,
                 distblas::core::SpMat<INDEX_TYPE,VALUE_TYPE> *sp_local_sender,
                 DenseMat<INDEX_TYPE, VALUE_TYPE> *dense_local,
                 DenseMat<INDEX_TYPE, VALUE_TYPE> *dense_local_output,
            ...
            cout << " rank "<< this->grid->rank_in_col  << " SpMMAlgo values "<<handle_2->values.size()<<" SpMMAlgo rowStart "<<handle_2->rowStart.size()<<" SpMMAlgo col "<<handle_2->col_idx.size()<< endl;
        ...
        void execute(int iterations, int batch_size, VALUE_TYPE lr) {
            ...
            for (int i = 0; i < iterations; i++) {
                ...
                for (int j = 0; j < batches; j++) {
                    ...
                    auto prevCoordinates_ptr = std::make_unique<std::vector<VALUE_TYPE>>(
                            batch_size * this->dense_local->cols, 0);
        ...
        inline void calc_embedding(INDEX_TYPE source_start_index,
            ...
            if (csr_block->handler != nullptr) {
                ...
                for (INDEX_TYPE i = dst_start_index; i <= dst_end_index; i++) {
                    ...
                    INDEX_TYPE local_dst = i - (grid)->rank_in_col *(this->sp_local_receiver)->proc_row_width;
                    ...
                    for (INDEX_TYPE j = static_cast<INDEX_TYPE>(csr_handle->rowStart[i]);
                        ...
                        if (csr_handle->col_idx[j] >= source_start_index and csr_handle->col_idx[j] <= source_end_index) {
                            ...
                            for (int d = 0; d < this->dense_local->cols; d++) {
                                if (!fetch_from_cache) {
                                    prevCoordinates[index * this->dense_local->cols + d] += lr * (this->dense_local)
                                            ->nCoordinates[local_dst * this->dense_local->cols + d];
                                } else {
                                    prevCoordinates[index * this->dense_local->cols + d] += lr * (array_ptr[d]);
                                }
                            }
                        }
                    }
                ...
                for (INDEX_TYPE i = source_start_index; i <= source_end_index; i++) {
                    ...
                    for (INDEX_TYPE j = static_cast<INDEX_TYPE>(csr_handle->rowStart[i]);
                        ...
                        if (dst_id >= dst_start_index and dst_id < dst_end_index) {
                            ...
                            for (int d = 0; d < this->dense_local->cols; d++) {
                                if (!fetch_from_cache) {
                                    prevCoordinates[index * this->dense_local->cols + d] += val * lr *
                                                                                            (this->dense_local)->nCoordinates[
                                                                                                    local_dst *
                                                                                                    this->dense_local->cols +
                                                                                                    d];
                                } else {
                                    prevCoordinates[index * this->dense_local->cols + d] += val * lr * array_ptr[d];
                                }
                            }
            ...
            for (int i = 0; i < (end_row - row_base_index); i++) {
                for (int d = 0; d < this->dense_local->cols; d++) {
                    (this->dense_local_output)
                            ->nCoordinates[(row_base_index + i) * this->dense_local->cols + d] =
                            prevCoordinates[i * this->dense_local->cols + d];
                }
            }
        }
    };
