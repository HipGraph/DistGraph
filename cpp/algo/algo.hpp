...
public:
  ...
  void algo_force2_vec_ns(int iterations, int batch_size, int ns, DENT lr) {
    ...
    auto full_comm = unique_ptr<DataComm<SPT, DENT, embedding_dim>>(new DataComm<SPT, DENT, embedding_dim>(
...
//              // remote computation for first batch
//              this->calc_t_dist_grad_rowptr(csr_block, prevCoordinates, lr, 0, batch_size, considering_batch_size,false, true, prev_start_proc,beta_prev_start, false);
//
//            } else if (k > beta_prev_start) {
//              // updating last remote fetched data vectors
//              int prev_end_process = get_end_proc(beta_prev_start, this->beta,this->grid->world_size);
//              this->calc_t_dist_grad_rowptr(csr_block, prevCoordinates, lr, 0,batch_size, considering_batch_size,false, true, beta_prev_start,prev_end_process, true);
...
inline void execute_pull_model_computations(std::vector<DataTuple<DENT, embedding_dim>> *sendbuf,
...
inline void calc_embedding_row_major(uint64_t source_start_index, uint64_t source_end_index,
        ...
        if (dst_id >= dst_start_index and dst_id < dst_end_index) {
          ...
          if (fetch_from_cache) {
            unordered_map<uint64_t, CacheEntry<DENT, embedding_dim>> &arrayMap =
                (true) ? (*this->dense_local->tempCachePtr)[target_rank]
                       : (*this->dense_local->cachePtr)[target_rank];
