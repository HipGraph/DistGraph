...
//          cout<<grid->global_rank << " cache  clearance completed"<<endl;
          ...
          if (alpha < 1.0) {
            ...
            for(int k=1;k<grid->world_size;k +=proc_length) {
              int end_process = get_end_proc(k,beta,grid->world_size);
              cout <<"rank "<<grid->global_rank<< " processing  " << k << " out of "<<grid->world_size<<" with proc length "<<proc_length<<" end process "<<end_process<<endl;
  ...
  inline void calc_t_dist_grad_for_cache_misses(vector<vector<Tuple<DENT>>> *cache_misses,
    ...
    for (int i = 0; i < sending_procs.size(); i++) {
      for (int k = 0; k < (*cache_misses)[sending_procs[i]].size(); k++) {
        ...
        DENT attrc = 0;
        DENT *array_ptr =
            (this->dense_local)->fetch_data_vector_from_cache(sending_procs[i], col_id,true);
        for (int d = 0; d < embedding_dim; d++) {
          forceDiff[d] =
            ...
            if (!matched) {
              if (fetch_from_cache) {
                array_ptr = (this->dense_local)
                                ->fetch_data_vector_from_cache(target_rank, i,temp_cache);
        ...
        for (uint64_t j = static_cast<uint64_t>(csr_handle->rowStart[i]);
          ...
          if (dst_id >= dst_start_index and dst_id <= dst_end_index) {
            ...
            if (fetch_from_cache) {
              array_ptr =
                  (this->dense_local)
                      ->fetch_data_vector_from_cache(target_rank, dst_id,false);
    ...
    for (int i = 0; i < block_size; i++) {
      ...
      for (int j = 0; j < col_ids.size(); j++) {
        ...
        if (fetch_from_cache) {
          ...
          DENT *colvec =
              (this->dense_local)
                  ->fetch_data_vector_from_cache(owner_rank, global_col_id,true);
          for (int d = 0; d < embedding_dim; d++) {
            forceDiff[d] =
