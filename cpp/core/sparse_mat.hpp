...
public:
  ...
  SpMat(Process3DGrid *grid, int &proc_row_width, const int &proc_col_width, bool hash_spgemm): DistributedMat() {
    ...
    if (hash_spgemm) {
      ...
      this->sparse_data_counter = make_unique<vector<INDEX_TYPE>>(proc_row_width, 0);
      this->hash_spgemm = true;
    }else{
      this->dense_collector = make_unique<vector<vector<VALUE_TYPE>>>(proc_row_width,vector<VALUE_TYPE>(proc_col_width,0));
    }
  }
