      ...
      for (uint64_t i = dst_start_index; i <= dst_end_index; i++) {
        ...
        for (uint64_t j = static_cast<uint64_t>(csr_handle->rowStart[i]);
          ...
          if (csr_handle->col_idx[j] >= source_start_index and
            ...
            if (!matched) {
              if (fetch_from_cache) {
                auto arrayMap = (temp) ? (*(this->dense_local->tempCachePtr))[target_rank] : (*((this->dense_local)->(cachePtr)))[target_rank];
...
//          (this->dense_local)->fetch_data_vector_from_cache(colvec,owner_rank, global_col_id, true);
          auto arrayMap = (temp) ? (*(this->dense_local)->(tempCachePtr))[owner_rank] : (*(this->dense_local)->(cachePtr))[owner_rank];
