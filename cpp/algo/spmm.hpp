...
public:
  SpMMAlgo(distblas::core::SpMat<SPT> *sp_local_native,
    ...
    //                                            batch_size, considering_batch_size, false, true, cache_misses_ptr.get(),
    ...
    //                update_ptr.get(), request_batch_update_cyclic, false, i, j, true);
    //            //            cout<<grid->global_rank << " processing  " << k << "
    //            //            out of "<<grid->world_size<<"population cache
    //            //            end"<<endl;
    //            prev_start = k;
    ...
    //            if (!(i == 0 and j == 0)) {
    //              data_comm_cache[j].get()->populate_cache(
    //                  update_ptr.get(), mpi_requests[i * batches + j - 1], false, i, j, false);
    //            }
    //            stop_clock_and_add(t, "Communication Time");
    //            t = start_clock();
    //          }
    ...
    //        int target_rank = (int)(i / (this->sp_local_receiver)->proc_row_width); bool fetch_from_cache =
