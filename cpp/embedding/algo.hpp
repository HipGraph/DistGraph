...
#pragma omp parallel for schedule(static)
    for (int i = 0; i < values.rows(); i++) {
      ...
      for (uint64_t j = static_cast<uint64_t>(csr_handle->rowStart[i]);
        ...
        ////        Eigen::Matrix<DENT, 1, embedding_dim> t_squared = t.array().pow(2); /        DENT t_squared_sum = t_squared.sum(); /        DENT d1 = -2.0 / (1.0 + t_squared_sum); /        Eigen::Matrix<DENT, 1, embedding_dim> scaled_vector = t * d1; / Eigen::Matrix<DENT, 1, embedding_dim> clamped_vector = /            scaled_vector.array().cwiseMax(this->MIN_BOUND).cwiseMin(this->MAX_BOUND);
        ////        Eigen::Matrix<DENT, 1, embedding_dim> learned = clamped_vector * lr; /        values.row(i) = values.row(i).array() + learned.array();
        //      }
      }
    }
  }
  ...
  void calc_t_dist_grad_repulsive(Matrix<DENT, Dynamic, embedding_dim> &values,
    ...
    for (int i = 0; i < values.rows(); i++) {
      ...
      for (int j = 0; j < col_ids.size(); j++) {
        ...
        int owner_rank = static_cast<int>(global_col_id /(this->sp_local)->proc_row_width);
